...
const Todos: React.FC = () => {
  ...
  // Generate printable HTML that will auto-save as PDF using html2pdf.js (CDN)
  const generatePrintableHtml = (todosToPrint: Todo[]) => {
    ...
    // Build rows with all requested data: budgetRequested, approvedBudget, cost, income and quarter ticks
    const rows = todosToPrint.map((t, i) => `
      <tr>
        ...
        <td>${escapeHtml(t.approvedBudget)}</td>
    ...
    // Include html2pdf CDN and a small script to auto-generate and save the PDF file.
    // The script runs after DOMContentLoaded, triggers html2pdf, then closes the window.
    const scripts = `
        ...
        (function() {
          ...
          document.addEventListener('DOMContentLoaded', function() {
            try {
              var opt = {
                margin:       10,
                filename:     sanitizeFilename('Annual_Arts_Plan_${escapeHtml(selectedYear || 'all')}_' + new Date().toISOString().replace(/[:.]/g, '-') + '.pdf'),
                image:        { type: 'jpeg', quality: 0.98 },
                html2canvas:  { scale: 2, logging: false, useCORS: true },
                jsPDF:        { unit: 'mm', format: 'a4', orientation: 'landscape' }
              };
              // wait a little for fonts/images to render
              setTimeout(function() {
  ...
  // Open printable window and let it auto-save PDF (html2pdf script in printable html will run)
  const exportFilteredToPdf = (todosToExport: Todo[]) => {
    const html = generatePrintableHtml(todosToExport);
    const w = window.open('', '_blank', 'noopener,noreferrer,width=1200,height=800');
    if (!w) {
      toast({ title: 'Pop-up blocked', description: 'Please allow pop-ups to export PDF.' });
      return;
    }
    w.document.open();
    ...
    <div className="max-w-full mx-auto p-6 bg-white dark:bg-card min-h-screen overflow-x-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => exportFilteredToPdf(filteredTodos)} className="gap-2">
            📄 Export PDF
          </Button>
        </div>
      </div>
        ...
        <table className="w-full border-collapse min-w-[1800px]">
          <thead>
            <tr className="bg-white dark:bg-card">
              ...
              <th rowSpan={2} className="border p-2 w-32">The budget requested by the department</th>
              <th rowSpan={2} className="border p-2 w-24">Approved budget</th>
              <th rowSpan={2} className="border p-2 w-20">Cost</th>
              <th rowSpan={2} className="border p-2 w-20">Income</th>
          ...
          <tbody>
            {filteredTodos.map((todo, index) => (
              <tr key={todo.id} className={`${todo.completed ? 'bg-green-50' : 'bg-white'} hover:bg-gray-50 cursor-pointer`} onDoubleClick={() => openEditDialog(todo)}>
                ...
                <td className="border p-2 text-center">{todo.approvedBudget}</td>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input value={formData.budgetRequested} onChange={(e) => setFormData(prev => ({ ...prev, budgetRequested: e.target.value }))} />
              </div>
              <div>
                <Label>Approved Budget</Label>
                <Input value={formData.approvedBudget} onChange={(e) => setFormData(prev => ({ ...prev, approvedBudget: e.target.value }))} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Cost</Label>
                <Input value={formData.cost} onChange={(e) => setFormData(prev => ({ ...prev, cost: e.target.value }))} />
              </div>
              <div>
                <Label>Income</Label>
                <Input value={formData.income} onChange={(e) => setFormData(prev => ({ ...prev, income: e.target.value }))} />
              </div>
            </div>
